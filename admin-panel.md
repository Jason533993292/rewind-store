# REWIND Admin Panel Code\n\n## Admin Panel (App.jsx lines 975-2431)\n\n```jsx
      </TweaksPanel>}
    </ClickSpark>
  );
}
...
```\n\n## Admin API Routes (api/server.js)\n\n```js
...
```\n\n## Prompt for Claude\n\n```\nAnalyze this admin panel code for a vintage streetwear e-commerce store (REWIND). The admin panel is at /#admin and allows managing users, orders, products, blocked emails, and sending campaigns.\n\nTell me:\n\n1. CRITICAL BUGS — logic errors that could break the admin panel\n2. SECURITY ISSUES — even with the current protections (server-side verify-admin, requireAdmin middleware, x-admin-token)\n3. UX IMPROVEMENTS — what would make managing the store easier\n4. MISSING FEATURES — what admin capabilities are needed but missing\n5. CODE QUALITY — messy patterns, dead code, inline styles that should be CSS classes\n\nBe specific — point to exact line numbers.\n```
