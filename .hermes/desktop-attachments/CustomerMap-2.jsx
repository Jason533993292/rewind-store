      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <svg viewBox="0 0 800 450" style={{ width: '100%', height: 'auto', overflow: 'visible' }}>
          ...
          {validLocations.map((loc, i) => {
            ...
            const r = Math.min(2 + Math.log(loc.count + 1) * 1.6, 6); // smaller, cleaner dots
            ...
            return (
              <g key={`${loc.city}-${loc.country}`}>
                ...
                {/* Soft static halo behind the dot for a bit of depth,
                    without the pulsing/blur of the previous version */}
                <circle cx={x} cy={y} r={r * 2.6} fill={color} opacity={0.15} />

                {/* Small solid core dot — also the tap target */}
                <circle
