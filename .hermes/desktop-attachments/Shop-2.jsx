...
export function Checkout({ open, items, onClose, onPlaced, userEmail, showToast, orderNumber: orderNumberProp, onInfo }) {
  ...
  async function handlePay() {
    ...
    // Declared here (not inside the branch below) so it's still in scope
    // when completeOrder() is called after the if/else block. Previously
    // this was declared with `const` INSIDE the `if` branch only, so
    // referencing it afterwards threw "payResult is not defined" on every
    // single payment attempt (card, Apple Pay, Google Pay) — the payment
    // itself would actually succeed with Stripe, but the app would then
    // crash right after, land in the catch block below, and show a
    // generic "Payment could not be processed" error instead of
    // confirming the order. This is the main bug behind "clicking Pay
    ...
    try {
      if (payment === 'card' || payment === 'applepay' || payment === 'googlepay') {
        ...
        // stripe.confirmPayment() needs to know which payment method to use.
        // We're not using the Payment Element (elements), so we must pass
        // payment_method_data explicitly — otherwise Stripe.js has no way
        // to tell what method to confirm and rejects the call before any
        // redirect happens. Each method also has its own minimum required
        // billing details.
        const billingBase = {
          name: formFields.name,
          email: formFields.email,
        ...
        // Redirect normally navigates the browser away before this point;
        // completeOrder() for this flow happens later, via the
        // orderNumberProp useEffect once the user returns from the
        // redirect. Nothing further to do here.
        setProcessing(false);
        return;
      }
