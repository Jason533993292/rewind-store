...
function Pulse() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const d = await apiGet('/api/settings/pulse');
      setData(d);
    } catch (e) {
      setError(e.message);
    }
    ...
    <div>
      ...
      {data.lowStock.length > 0 && (
        <div style={{ marginTop: '6px' }}>
          <p style={{ fontSize: '12px', fontWeight: 600, color: '#E11D74', margin: '0 0 8px' }}>
            ⚠ Low stock ({data.lowStock.length})
          </p>
          {data.lowStock.map((p, i) => (
            <div key={p.id || i} style={{
              display: 'flex', justifyContent: 'space-between', alignItems: 'center',
              padding: '6px 0', fontSize: '13px', borderBottom: '1px solid var(--line)',
            }}>
              <span style={{ color: 'var(--ink)', fontWeight: 500 }}>{p.name}</span>
              <span style={{
                ...SP_STYLES.tag, background: p.stock === 0 ? '#FEE2E2' : '#FEF3C7',
      ...
      {data.lowStock.length === 0 && (
        <p style={{ fontSize: '13px', color: '#059669', margin: '6px 0 0' }}>✅ All items fully stocked</p>
      )}

      <div style={{ marginTop: '10px' }}>
        <button onClick={load} style={{
          ...SP_STYLES.btnSmall, background: 'none', color: 'var(--muted)',
          border: '1px solid var(--line)', padding: '4px 12px', fontSize: '11px',
        }}>
          ↻ Refresh
        </button>
      </div>
...
function OrderLookup() {
  ...
  const search = useCallback(async () => {
    ...
    setSearched(true);
    try {
      const d = await apiGet('/api/settings/order-lookup?q=' + encodeURIComponent(q.trim()));
      setOrders(d.orders || []);
    } catch {
      setOrders([]);
    }
  ...
  return (
    <div>
      <div style={{ display: 'flex', gap: '8px' }}>
        <input
          style={SP_STYLES.input}
          placeholder="Order number…"
          value={q}
          onChange={e => setQ(e.target.value)}
          onKeyDown={e => { if (e.key === 'Enter') search(); }}
        />
        <button onClick={search} disabled={loading || !q.trim()} style={SP_STYLES.btn}>
          {loading ? '…' : 'Search'}
        </button>
      </div>
      ...
      {orders.map((o, i) => (
        <div key={o.order_num || i} style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
            ...
            <span style={{ ...SP_STYLES.tag, background: statusColor(o.status) + '18', color: statusColor(o.status) }}>
              {o.status}
            </span>
          </div>
          <p style={{ fontSize: '12px', color: 'var(--muted)', margin: '0 0 4px' }}>
            {o.customer_name || '—'} · {o.email || '—'}
          </p>
          <p style={{ fontSize: '12px', color: 'var(--muted)', margin: '0' }}>
            {Array.isArray(o.items) ? o.items.map(it => it.name || it.product_id || 'Item').join(', ') : '—'}
          </p>
          <p style={{ fontSize: '13px', fontWeight: 600, color: 'var(--ink)', margin: '6px 0 0' }}>
...
function PromoGenerator({ showToast }) {
  ...
  return (
    <div>
      <div style={{ marginBottom: '14px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' }}>
          ...
          <input
            ...
            style={{ flex: 1, accentColor: 'var(--accent, #FF4D14)' }}
          />
        </div>
      ...
      <button onClick={generate} disabled={loading} style={SP_STYLES.btn}>
      ...
      {generated && (
        <div style={{
          ...
          <div>
            <span style={{ fontSize: '16px', fontWeight: 700, color: 'var(--ink)', letterSpacing: '0.5px' }}>
              {generated.code}
            </span>
          ...
          <button onClick={handleCopy} style={SP_STYLES.btnSmall}>
            {copied ? '✓ Copied' : 'Copy'}
          </button>
        </div>
      )}
...
export default function SettingsPanel({ onClose, showToast }) {
  ...
  return (
    <div style={SP_STYLES.overlay}>
      <div style={SP_STYLES.header}>
        <button
          ...
          style={{
            ...
            color: 'var(--ink)', padding: '4px', display: 'flex',
          }}
      ...
      {/* Tab buttons for section switching */}
      <div style={{
        ...
        {sections.map(s => (
          <button key={s.id} onClick={() => setActiveSection(s.id)} style={{
            ...
            background: activeSection === s.id ? 'var(--ink)' : 'var(--surface)',
            color: activeSection === s.id ? 'var(--surface)' : 'var(--muted)',
            borderBottom: activeSection === s.id ? 'none' : '1px solid var(--line)',
      ...
      <div style={SP_STYLES.body}>
        {activeSection === 'pulse' && (
          <div style={SP_STYLES.card}>
            <h3 style={SP_STYLES.cardTitle}>📊 Store Pulse</h3>
            <Pulse />
          </div>
        )}
        ...
        {activeSection === 'lookup' && (
          <div style={SP_STYLES.card}>
            <h3 style={SP_STYLES.cardTitle}>🔍 Order Lookup</h3>
            <OrderLookup />
          </div>
        )}
        ...
        {activeSection === 'promo' && (
          <div style={SP_STYLES.card}>
            <h3 style={SP_STYLES.cardTitle}>🎁 Promo Generator</h3>
            <PromoGenerator showToast={showToast} />
          </div>
        )}
      </div>
    </div>
