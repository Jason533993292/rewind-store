...
## [DONE] Checkout component exposes hardcoded test data — every production user sees fake customer info and Stripe test card numbers
- **Where:** `src/components/Shop.jsx` lines 478–484 and 500–505 — the Checkout component's delivery and card fields
- **What:** The checkout form had hardcoded `defaultValue` props: `"Alex R."` (name), `"Kerkstraat 42"` (address), `"1000"` (postal code), `"Brussels"` (city), `"Belgium"` (country), `"4242 4242 4242 4242"` (card number), `"12 / 27"` (expiry), `"123"` (CVC). Every customer who opened checkout saw these pre-filled values — including the Stripe test card number. The test data was originally put there for Playwright convenience but was visible to all real users on production.
- **Why it matters:** (1) **Privacy concern** — a fake identity's personal info is shown to every user. (2) **Misleading UX** — users see pre-filled data and may think it's their own info or the store's default. (3) **Stripe test card exposure** — a user could accidentally attempt to pay with the test card number. (4) **Unprofessional first impression** — seeing "Alex R." at Kerkstraat 42 in a checkout form erodes trust.
- **Fix:** Replaced all 8 `defaultValue` values with `defaultValue=""`. The Playwright test (`tests/comprehensive.spec.js`) now fills these fields explicitly via `.fill()` before clicking Pay, so the test continues to work locally. (V6.5.57)
